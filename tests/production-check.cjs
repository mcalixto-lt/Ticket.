...
assert(sw.includes("ticket-app-v190"));
assert(!sw.includes('?v=167'));
assert.strictEqual(JSON.parse(read('version.json')).version,'1.0.90');
assert.strictEqual(JSON.parse(read('public/version.json')).version,'1.0.90');
