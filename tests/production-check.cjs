...
assert(html.includes('ticket-journey-complete-v192.js?v=192'));
assert(!html.includes('?v=167'));
assert(sw.includes("ticket-app-v192"));
assert(!sw.includes('?v=167'));
assert.strictEqual(JSON.parse(read('version.json')).version,'1.0.92');
assert.strictEqual(JSON.parse(read('public/version.json')).version,'1.0.92');
...
const journeyComplete=read('src/ticket-journey-complete-v192.js');
assert(journeyComplete.includes('Parabéns, você concluiu sua jornada!'));
assert(journeyComplete.includes('Bom descanso.'));
assert(journeyComplete.includes('calculateRecord(after,state.schedule).complete'));
assert(journeyComplete.includes('afterCount<=beforeCount'));
